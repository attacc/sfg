...
\begin{abstract}
Sum frequency generation (SFG) is a second order non-linear process where two laser with frequencies $\omega_1$ and $\omega_2$ combine to produce a reponse at frequency $\omega=\omega_1 + \omega_1$. Due to the freedom in the choise of the two laser frequencies respect to other non-linear responses as second harmonic generation, different responces can be exploited to enhance the non-linear reponse. In this manuscript we present a first-principle approch based on a real-time electron dynamics that allows to include excitonic effect in the non-linear reponse functions.  
...
Sum frequency generation (SFG) with more explicit phase-matched linear combination of two different frequencies was reported in layered MoS$_2$ and related heterosystems using  band-filtered  supercontinuum  illumination.\cite{li2016multimodal} SFG  from monolayer  and heterobilayer  TMD structures has be also utilized for the wavelength-dependent spectroscopy.\cite{yao2019continuous,wang2020difference,PhysRevB.107.155433} Recently it has been show the possibilit to make SFG dual resonant with exciton in two materials, strongly enhancing this response function.\cite{kim2020dual} (ADD MORE)
From the theoretical point of view, SFG was mainly investigated using real-time evolution of the nonlinear current in  two-band model(CITE SOMEONE) and the real-time time-dependent density functional theory.\cite{kim2020dual}
